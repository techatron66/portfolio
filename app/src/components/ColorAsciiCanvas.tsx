...
// Get hue based on intensity for colorful moon
const getMoonHue = (intensity: number): number => {
  if (intensity > 0.7) {
    // Bright areas: amber/gold (40-60)
    return 45 + (intensity - 0.7) / 0.3 * 15;
  } else if (intensity > 0.35) {
    // Mid areas: cyan (170-190)
    return 185 - (intensity - 0.35) / 0.35 * 15;
  } else {
    // Shadow areas: magenta/purple (280-320)
    return 320 - intensity / 0.35 * 40;
  }
};

const getMoonSaturation = (intensity: number): number => {
  if (intensity > 0.7) return 100;
  if (intensity > 0.35) return 90;
  return 75;
};

// Get field color based on density and position
const getFieldColor = (density: number, normX: number, normY: number): [number, number, number, number] => {
  const hue = 180 + density * 60 + normX * 30 + Math.sin(normY * 6) * 20;
  const sat = 60 + density * 30;
  const light = 40 + density * 30;
  const alpha = 0.06 + density * 0.34;
  return [hue % 360, sat, light, alpha];
};

export default function ColorAsciiCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp = 0) => {
      ...
      const moonRadius = Math.min(width, height) * 0.32;
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
          ...
          if (normMoon < 1.0) {
            ...
            const hue = getMoonHue(intensity);
            const sat = getMoonSaturation(intensity);
            const light = 40 + intensity * 35;

            // Neon glow on bright characters
            if (intensity > 0.6) {
              ctx.shadowColor = hsl(hue, sat, 50, 1);
              ctx.shadowBlur = intensity * 8;
            } else {
              ctx.shadowBlur = 0;
            }

            ctx.fillStyle = hsl(hue, sat, light, opacity);
            ...
            if (density > 0.3) {
              ...
              opacity = 0.07 + density * 0.34;

              // COLOR for field characters
              const [fh, fs, fl, fa] = getFieldColor(density, c / cols, r / rows);
              ...
              if (char && opacity > 0.02) {
                ctx.shadowColor = hsl(fh, fs, 50, fa * 2);
                ctx.shadowBlur = 2;
                ctx.fillStyle = hsl(fh, fs, fl, fa);
                ctx.fillText(char, drawX, drawY);
                ctx.shadowBlur = 0;
              }
            }
